...
;;; BASIC SETUP AN INSTRUCTIONS FOR tmux + vim-slime
; start tmux: run 'tmux' in terminal
; create new vertically split pane: ctrl+b -> %
; resize vertically split pane (set width of 100 characters): ctrl+b -> :resize-pane -x 100
; start steel bank common lisp environemt: run 'sbcl' in pane
; change active pane: ctrl+b -> left/right arrow

; using vim-slime
; ctrl+c -> ctrl+c to sent code block (code with no empty rows) into REPL (can use visual selection) (first rune of REPL may require some setup (specified in init.vim))
